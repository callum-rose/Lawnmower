				...
				LevelEditorWindow levelEditorWindow)
			{
				...
				foreach (int y in levelData.GetYIndices())
				{
					foreach (int x in levelData.GetXIndices())
					{
						...
						if (levelEditorWindow._mowerPosition != null && levelEditorWindow._mowerPosition.CurrentPosition.Value == position)
						{
							DrawMower(cellRect);
						}
						
						if (Event.current.OnLeftClick(cellRect, false) || (Event.current.type == EventType.MouseDrag && cellRect.Contains(Event.current.mousePosition)))
						{
							Event.current.Use();
