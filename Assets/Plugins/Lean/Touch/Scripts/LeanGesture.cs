...
namespace Lean.Touch
{
	...
	public static class LeanGesture
	{
		...
		{
			float scale = 1.0f;
			Vector2 center = GetScreenCenter(fingers);
			Vector2 lastCenter = GetLastScreenCenter(fingers);
		...
		{
			float distance = GetScreenDistance(fingers, center);
			float lastDistance = GetLastScreenDistance(fingers, lastCenter);
			...
			{
				float scroll = LeanInput.GetMouseWheelDelta();
		...
		{
			Vector2 center = LeanGesture.GetScreenCenter(fingers);
			Vector2 lastCenter = LeanGesture.GetLastScreenCenter(fingers);
